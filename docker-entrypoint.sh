...
# Check if vulnerabilites related to the severity threshold are found and if so fail action
  case $INPUT_SEVERITY_THRESHOLD in
	critical)
        CRITICAL_VULNS_FOUND=$(cat ${GITHUB_WORKSPACE}/evaluations/${IMAGE_NAME}/${IMAGE_TAG}/evaluation_*.json | jq '.cve.fixable_vulnerabilities')
		...
		;;
	high)
        HIGH_VULNS_FOUND=$(cat ${GITHUB_WORKSPACE}/evaluations/${IMAGE_NAME}/${IMAGE_TAG}/evaluation_*.json | jq '.cve.high_vulnerabilities')
		...
		;;
	medium)
        MEDIUM_VULNS_FOUND=$(cat ${GITHUB_WORKSPACE}/evaluations/${IMAGE_NAME}/${IMAGE_TAG}/evaluation_*.json | jq '.cve.medium_vulnerabilities')
		...
		;;
	low)
        LOW_VULNS_FOUND=$(cat ${GITHUB_WORKSPACE}/evaluations/${IMAGE_NAME}/${IMAGE_TAG}/evaluation_*.json | jq '.cve.low_vulnerabilities')
		...
		;;
	info)
        INFO_VULNS_FOUND=$(cat ${GITHUB_WORKSPACE}/evaluations/${IMAGE_NAME}/${IMAGE_TAG}/evaluation_*.json | jq '.cve.INFO_vulnerabilities')
