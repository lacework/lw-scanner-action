...
if [ ${INPUT_USE_POLICY} = "true" ]; then
    export SCANNER_PARAMETERS="${SCANNER_PARAMETERS} --policy --fail-on-violation-exit-code 11 --critical-violation-exit-code 12 --high-violation-exit-code 13 --medium-violation-exit-code 14 --low-violation-exit-code 15 --info-violation-exit-code 16"
fi
